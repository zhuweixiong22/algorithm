...
$F(0, j, k) = 0$ ：表示任何一个物品都不选，体积不超过j，重量不超过k是一个合法方案，且价值为0
...
将整个集合拆分为和相等的两个子集，所以背包容量为 $sum / 2$, 对于每个数 $i$ 来说，$i$ 要么存在于集合1，要么存在于集合2，所以我们表示其中一个集合的状态即可，只要确定其中一个集合的数，那么另一个集合的数就确定了，最终答案为方案数除以 $2$，因为是两个集合的组合问题。

**状态定义：**

$F(n, sum / 2)：$ 考虑选取 $1 \sim i$ 个数字，子集1和恰好为 $sum / 2$ 的所有方案数
...
$F(0, 0) = 1 $ 什么物品都不选恰好能装满容量为0的背包是一个合法方案
...
#### 题目描述
[AcWing1020. 潜水员](https://www.acwing.com/problem/content/description/1022/)
...
题目要求氧气含量 $\geq m$ 和氮气含量 $\geq n$ 所需要气缸重量的最小值

而二维费用背包求的是代价1 $\leq m$ 某个上限, 代价2 $\leq m$ 某个上限，能够获取的最大价值。

所以要修改一下二维费用0-1背包的状态定义

**状态定义：**

$F(count, m, n)$：考虑选择 $1 \sim count$ 个气缸, 且氧气含量 $\geq m$ 和氮气含量 $\geq n$ 所需要气缸重量的最小值

**状态转移：**

$F(i, j, k) = max(F(i - 1, j, k), w(i) + F(i, max(0, j - v(i)), max(0, k - m(i))))$ 下面解释这个状态转移

**初始状态：**

与二维费用0-1背包第一个区别：

初始值不同，二维费用背包什么物品都不选 $F(0, j, k) = 0$ ：是一个合法值，二维费用是代价不超过某个值，而这里的状态是大于等于某个值，所以只有 $F(0, 0, 0) = 0$ 才是合法值，那么如何去体现 $F(0, j, k) (j \neq 0, k \neq 0)$ 的不合法，因为求的是最小值，常用技巧就是将它赋为一个正无穷即可，或者赋值成-1，状态转移时进行特判。

第二个区别：

状态转移方程有些变化，分析下：为什么在 0-1 背包的时候， v1[i] > j 和 v2[i] > k 不能作为取值，因为我们背包的容量只有 j 和 k，放不下 v1[i] 和 v2[i]。但是在这里，假设我们把 v1[i] 和 v2[i] 装进背包，并不会有什么问题，因为要求的就是背包1容量大于等于m，背包2容量大于等于n。如果不装进来反而会漏更新状态。因为状态不能为负数，所以等价于 0 ，下一状态都为 $F(i, j, k)$。

```java
// 状态转移
if (v1[i] > j && v2[i] > k) {
    dp[i][j][k] = Math.min(dp[i - 1][j][k], dp[i - 1][0][0] + w[i]);
} else if (v1[i] > j) {
    dp[i][j][k] = Math.min(dp[i - 1][j][k], dp[i - 1][0][k - v2[i]] + w[i]);
} else if (v2[i] > k) {
    dp[i][j][k] = Math.min(dp[i - 1][j][k], dp[i - 1][j - v1[i]][0] + w[i]);
} else {
    dp[i][j][k] = Math.min(dp[i - 1][j][k], dp[i - 1][j - v1[i]][k - v2[i]] + w[i]);
}

// 上面的状态转移可以简化为：

dp[i][j][k] = Math.min(dp[i - 1][j][k], dp[i - 1][Math.max(0, j - v1[i])][Math.max(0, k - v2[i])] + w[i]);
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[] v1 = new int[count + 1];
        int[] v2 = new int[count + 1];
        ...
        System.out.println(dp[count][m][n]);
    }
}
```
...
for (int i = 1; i <= count; i++) {
    for (int j = m; j >= 0; j--) {
        for (int k = n; k >= 0; k--) {
            dp[j][k] = Math.min(dp[j][k], dp[Math.max(0, j - v1[i])][Math.max(0, k - v2[i])] + w[i]);
        }
    }
}
